...
class Long {
  ...
  /**
   * GCD between current number and b
   * @param {Long} b Number
   * @returns Long
   */
  gcd(b) {
    let a = this
    while (a.notEquals(ZERO_) && b.notEquals(ZERO_)) {
      while (b.and(ONE_).equals(ZERO_)) {
        b = b.shiftRight(1)
      }
      while (a.and(ONE_).equals(ZERO_)) {
        a = a.shiftRight(1)
      }
      if (a.compare(b) > 0) {
        a = a.subtract(b)
...
function getCachedIntValue_(value) {
  return IntCache_[value] || (IntCache_[value] = new Long(value, value < 0 ? -1 : 0))
}
