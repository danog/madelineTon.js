...
function longToInts(sLong) {
    var divRem = bigStringInt(sLong).divideAndRemainder(bigiint(0x100000000))
...
function longToBytes(sLong) {
    return bytesFromWords({
        words: longToInts(sLong),
        sigBytes: 8
    }).reverse()
}

function longFromInts(high, low) {
    return bigiint(high).shiftLeft(32).add(bigiint(low)).toString(10)
}
...
function nextRandomInt(maxValue) {
    let n = Math.floor(Math.random() * maxValue)
    vals.push(n)
    return n
}
function nextFakeRandomInt() {
    return vals[idx++]
}
idx = 0

function pqPrimeFactorization(pqBytes) {
    var what = bigStringInt(pqBytes)

    result = pqPrimeLong(Long.fromString(pqBytes, 10))
    nextRandomInt = nextFakeRandomInt
    idx = 0
    ...
    if (typeof BigInt !== 'undefined') {
        console.time('pq native')
        result = factorize(BigInt(pqBytes))
        console.timeEnd('pq native')
        console.log(result)
    }
    ...
    console.time('pq three')
    result = pqPrimeBigInteger(what)
    console.timeEnd('pq three')
